...
class FavoriteViewModel @Inject constructor(
    ...
    private fun getMusicList() = scope.launch {
        ...
        try {
            _musicList.tryEmit(
                api.getMusicList("Bearer ${storageController.getAccessToken()}")
                    .filter { _favorites.value.contains(it.composition_id) })
    ...
    fun removeFavorite(id: Int) {
        ...
        _musicList.tryEmit(_musicList.value.filter { _favorites.value.contains(it.composition_id) })
        storageController.setFavorites(_favorites.value)
    }
